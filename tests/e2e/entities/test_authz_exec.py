...
from cosmpy.protos.cosmos.staking.v1beta1.authz_pb2 import StakeAuthorization, AUTHORIZATION_TYPE_DELEGATE

repo_root_path = Path(__file__).parent.parent.parent.parent.absolute()
sys.path.insert(0, str(repo_root_path))

from tests.helpers.entity_test import EntityTest
from tests.helpers.field_enums import MsgFields, AuthzExecFields, AuthzExecMessageFields
...
class TestAuthzExec(EntityTest):
    ...
    def setUpClass(cls):
        ...
        grantee_mnemonic = Bip39MnemonicGenerator().FromWordsNumber(Bip39WordsNum.WORDS_NUM_24)
        ...
        tx = Transaction()
        tx.add_message(MsgGrant(
            granter=cls.delegator_address,
            grantee=cls.grantee_address,
            grant=(Grant(authorization=authorization_any,
        ...
        msg_begin_delegate = create_delegate_msg(delegator=cls.delegator_address,
    ...
    def test_exec_delegate(self):
        ...
        authz_exec_messages = self.db_cursor.execute(
            AuthzExecMessageFields.select_query()).fetchall()
        ...
        for authz_exec in authz_execs:
            ...
            self.assertEqual(authz_exec[AuthzExecFields.grantee.value], self.grantee_address)

        for authz_exec_message in authz_exec_messages:
            found_related_message = False
            for message in messages:
                if message[MsgFields.id.value] == authz_exec_message[AuthzExecFields.message_id.value]:
    ...
    def test_blocks_query(self):
        ...
        self.assertEqual(sub_message["message"]["typeUrl"], "/cosmos.staking.v1beta1.MsgDelegate")
